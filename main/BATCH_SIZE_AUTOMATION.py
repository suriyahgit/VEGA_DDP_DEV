...
INFERENCE_BATCH_SIZE = 8192  # Increased from 4096
NUM_WORKERS = 8
EARLY_STOPPING_PATIENCE = 25
MIN_LR = 1e-6
GRADIENT_ACCUMULATION_STEPS = 2  # Number of micro-batches
...
def train(rank, world_size, data):
    ...
    logger.info(f"Initializing training process on rank {rank}")
    ...
    logger.info(f"Model created on device {rank}")
        ...
        while batch_size >= 256:  # Don't go below 256
            try:
                ...
                if 'out of memory' in str(e):
                    ...
                    torch.cuda.empty_cache()
                    logger.warning(f"Reducing batch size to {batch_size} due to OOM")
                else:
                    raise
        return batch_size
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if rank == 0:
            ...
            if current_lr < MIN_LR:
                logger.info(
                    f"Learning rate below minimum threshold ({current_lr:.2e} < {MIN_LR:.2e}), "
                    "stopping training"
                )
                break
