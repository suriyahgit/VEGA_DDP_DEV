...
class XarrayWeatherDataset(Dataset):
    def __init__(self, data, patch_size=3, time_steps=5, num_tiles_per_time=500):
        ...
        # Pad data with Dask
        self.data = self._pad_data_dask(data)
        
        # Generate indices with chunked processing
        self.indices = self._generate_indices()

    def _pad_data_dask(self, data):
        """Memory-efficient padding with Dask"""
        logger.info(f"Applying Dask padding to data of shape {data.shape}")
        
        # Convert to Dask array with optimal chunking
        data_dask = da.from_array(data, chunks=(100, -1, -1, -1))  # Chunk along time dimension
        ...
        padded_data = da.zeros(padded_shape, dtype=data.dtype, chunks=(100, -1, -1, -1))
        
        # Fill central region
        padded_data[self.time_steps-1:, self.pad:-self.pad, self.pad:-self.pad] = data_dask
        
        # Handle periodic boundaries
        padded_data[self.time_steps-1:, :self.pad] = data_dask[:, -self.pad:]
        padded_data[self.time_steps-1:, -self.pad:] = data_dask[:, :self.pad]
        ...
        # Compute the result
        with ProgressBar():
            logger.info("Computing padded data...")
            padded_data = padded_data.compute()
        
        return padded_data

    def _generate_indices(self):
        """Generate indices with Dask-delayed computation"""
        from dask import delayed
        
        logger.info("Generating indices with Dask...")
        ...
        def generate_chunk(t_start, t_end):
            ...
            for t in range(t_start, t_end):
                ...
                chunk_indices.extend([(t, lat[i], lon[i]) for i in range(self.num_tiles_per_time)])
        ...
        for t_start in range(self.time_steps - 1, total_time, chunk_size):
            ...
            tasks.append(generate_chunk(t_start, t_end))
        ...
        indices = [idx for chunk in results for idx in chunk]
...
class FullXarrayWeatherDataset(Dataset):
    def __init__(self, data, patch_size=3, time_steps=5):
        self.data = self._pad_data_dask(data, patch_size, time_steps)
        self.patch_size = patch_size
        self.time_steps = time_steps
        self.valid_time = data.shape[0]
        self.H, self.W = data.shape[1], data.shape[2]
        self.indices = self._generate_indices_dask()

    def _pad_data_dask(self, data, patch_size, time_steps):
        """Dask-optimized padding for inference"""
        pad = patch_size // 2
        data_dask = da.from_array(data, chunks=(100, -1, -1, -1))
        ...
        padded_data = da.zeros(padded_shape, dtype=data.dtype, chunks=(100, -1, -1, -1))
        padded_data[time_steps-1:, pad:-pad, pad:-pad] = data_dask
        
        # Periodic boundaries
        padded_data[time_steps-1:, :pad] = data_dask[:, -pad:]
        padded_data[time_steps-1:, -pad:] = data_dask[:, :pad]
        ...
        with ProgressBar():
            logger.info("Computing inference padded data...")
            return padded_data.compute()

    def _generate_indices_dask(self):
        """Dask-optimized index generation"""
        from dask import delayed
        ...
        with ProgressBar():
            logger.info("Computing inference indices...")
            results = dask.compute(*tasks)
