...
# --- Modified Dataset Class with Caching ---
class CachedXarrayWeatherDataset(Dataset):
    def __init__(self, data, patch_size=PATCH_SIZE, time_steps=TIME_STEPS, num_tiles_per_time=NUM_TILES_PER_TIME):
        self.logger = logging.getLogger(__name__)
        self.logger.info("Initializing CACHED dataset with patch_size=%d, time_steps=%d", patch_size, time_steps)
        
        # Original data remains on CPU/numpy
        self.data = data
        ...
        # Cache storage
        self.cache = {}  # Will store {idx: tensor_on_gpu}
        self.cache_order = []  # For LRU eviction if needed
    ...
    def __getitem__(self, idx):
        ...
        tensor_patch = torch.tensor(patch, dtype=torch.float32).flatten()
        
        # Cache on first access (moved to GPU in DataLoader)
        self.cache[idx] = tensor_patch
        self.cache_order.append(idx)
        
        # Optional: Implement LRU eviction if memory constrained
...
def create_dataloader(dataset, rank, world_size):
    ...
    return DataLoader(
        ...
        pin_memory=True,           # Critical for GPU retention
...
def train(rank, world_size, data):
    ...
    # Create cached dataset and dataloader
    train_dataset = CachedXarrayWeatherDataset(data)
    ...
    logger.info("DataLoader initialized with batch_size=%d, num_workers=%d, prefetch=%d", 
    ...
    for epoch in range(1000):  # Large number for early stopping
        ...
        if epoch == 0:
            ...
            logger.info("Initial cache warming epoch - may be slower")
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            if batch_idx % 100 == 0 and rank == 0:
                ...
                cached_batches = len(train_dataset.cache) // BATCH_SIZE
                logger.info(
                    f"Epoch {epoch+1} | Batch {batch_idx} | "
                    f"Loss: {loss.item():.4f} | "
                    f"GPU Mem: {current_mem:.2f}GB | "
                    f"Cached: {cached_batches} batches"
                )
        ...
        if rank == 0:
            ...
            if avg_loss < best_loss:
                ...
                patience_counter += 1
                logger.info(
                    "No improvement (%d/%d), Current Loss: %.4f, Best Loss: %.4f", 
            ...
            if current_lr < MIN_LR:
                logger.info(
                    "Learning rate below minimum threshold (%.2e < %.2e), stopping training", 
                    current_lr, MIN_LR
                )
                break
    ...
    logger.info(
        "Training completed in %.2f minutes (%.2f hours)", 
        training_time/60, training_time/3600
    )
    dist.destroy_process_group()
    
    # Explicit cleanup of cache (optional)
    if rank == 0:
