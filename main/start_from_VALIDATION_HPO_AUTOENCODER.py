...
def train(rank, world_size, data):
    ...
    # Check for existing model (only on rank 0 to avoid race conditions)
    ...
    # Wrap model in DDP after potential loading
    ...
    # If we loaded a model, we should get its validation loss to compare against
    if rank == 0 and os.path.exists(model_path):
        model.eval()
        total_val_loss = 0
        with torch.no_grad():
        ...
        logger.info("Loaded model's initial validation loss: %.4f", best_val_loss)
        model.train()
