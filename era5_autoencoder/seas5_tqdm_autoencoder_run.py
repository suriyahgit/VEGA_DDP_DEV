...
def train(rank, world_size, data, model_path=None, output_zarr_path=None, batch_size=DEFAULT_BATCH_SIZE, 
    ...
    for epoch in range(1000):  # Large number for early stopping
        ...
        train_loop = tqdm(train_loader, desc=f"Epoch {epoch+1}", leave=False, disable=rank!=0)
        total_train_loss = 0
        ...
        model.eval()
        total_val_loss = 0
        val_loop = tqdm(val_loader, desc="Validating", leave=False, disable=rank!=0)
...
def run_inference(data, model_path="best_model.pth", output_zarr_path=None, batch_size=DEFAULT_BATCH_SIZE, 
    ...
    # Check if data has ensemble dimension (shape should be [number, time, lat, lon, variable] if present)
    has_ensemble = len(data.shape) == 5
    if has_ensemble:
        logger.info("Running inference on ensemble data with shape %s", data.shape)
        num_members = data.shape[0]
    else:
        logger.info("Running inference on single member data with shape %s", data.shape)
        num_members = 1
    ...
    for member_idx in range(num_members):
        if has_ensemble:
            member_data = data[member_idx]
            logger.info("Processing ensemble member %d/%d", member_idx+1, num_members)
        else:
            member_data = data
        
        full_dataset = FullXarrayWeatherDataset(member_data)
        ...
        member_latents = []
        with torch.no_grad():
            inference_loop = tqdm(full_loader, desc=f"Inference for member {member_idx+1}" if has_ensemble else "Inference")
            for batch_idx, batch in enumerate(inference_loop):
                batch = batch.to(device, non_blocking=True)
                _, latents = model(batch)
                member_latents.append(latents.cpu())
                inference_loop.set_postfix(batch=batch_idx)
                
                if batch_idx % 100 == 0:
                    logger.info("Processed %d/%d batches for member %d", 
                              batch_idx, len(full_loader), member_idx+1 if has_ensemble else 1)
        
        member_latents = torch.cat(member_latents, dim=0)
        
        # Calculate expected shape
        valid_time = member_data.shape[0] - TIME_STEPS + 1
        H, W = member_data.shape[1], member_data.shape[2]
        expected_elements = valid_time * H * W * LATENT_DIM
        
        if member_latents.numel() != expected_elements:
            logger.error("Shape mismatch for member %d! Got %d elements but expected %d", 
                       member_idx+1 if has_ensemble else 1, member_latents.numel(), expected_elements)
            raise ValueError(f"Shape mismatch for member {member_idx+1 if has_ensemble else 1}")
        
        # Reshape and pad
        member_latents_np = member_latents.numpy().reshape(valid_time, H, W, LATENT_DIM)
        pad_front = np.repeat(member_latents_np[[0]], TIME_STEPS - 1, axis=0)
        full_latents = np.concatenate([pad_front, member_latents_np], axis=0)
        
        all_member_latents.append(full_latents)
    
    # Stack all members along new dimension
    if has_ensemble:
        full_latents = np.stack(all_member_latents, axis=0)
    else:
        full_latents = all_member_latents[0]
    ...
    if output_zarr_path:
        if has_ensemble:
            number_coords = np.arange(data.shape[0])
            time_coords = np.arange(data.shape[1])
            lat = np.arange(data.shape[2])
            lon = np.arange(data.shape[3])
            ...
            for i in range(full_latents.shape[-1]):
                ...
                variables[var_name] = xr.DataArray(
                    full_latents[..., i],
                    dims=["number", "time", "lat", "lon"],
                    coords={"number": number_coords, "time": time_coords, "lat": lat, "lon": lon}
                )
        else:
...
def preprocess_data(input_zarr_path, variable_type):
    ...
    if has_ensemble:
        logger.info("Dataset contains ensemble members (number dimension)")
    ...
    if has_ensemble:
        ...
        for member in ds.number.values:
            logger.info(f"Processing ensemble member {member}")
            ...
            for var in variables:
                ...
                if variable_type == 'tp' and var == 'precip_binary':
                    ...
                    with ProgressBar():
                        normalized_array = member_ds[var].compute().expand_dims('variable')
                else:
                    # Normalize this variable for this member
                    with ProgressBar():
                        var_mean = member_ds[var].mean(dim=['time', 'lat', 'lon']).compute()
                        var_std = member_ds[var].std(dim=['time', 'lat', 'lon']).compute()
                    
                    logger.info(f"Computed mean for {var}: {var_mean.values}")
                    logger.info(f"Computed std for {var}: {var_std.values}")
                    
                    normalized_var = (member_ds[var] - var_mean) / (var_std + 1e-8)
                ...
                member_normalized.append(normalized_array)
                del normalized_array
            
            # Combine variables for this member
            member_data = xr.concat(member_normalized, dim='variable')
            member_data = member_data.transpose('time', 'lat', 'lon', 'variable')
            normalized_chunks.append(member_data.expand_dims('number'))
        
        # Combine all members
        data = xr.concat(normalized_chunks, dim='number')
    else:
        # Original single-member processing
        normalized_chunks = []
        for var in variables:
            logger.info(f"Processing variable: {var}")
