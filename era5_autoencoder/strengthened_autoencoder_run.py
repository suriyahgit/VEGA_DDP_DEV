...
def train(rank, world_size, data, model_path=None, output_zarr_path=None, batch_size=DEFAULT_BATCH_SIZE, 
          ...
          model_type='mlp', unet_channels=32):
    ...
    if model_type == 'mlp':
        ...
        input_channels = TIME_STEPS * data.shape[-1]  # Channels = time_steps * variables
        model = WeatherUNet(input_channels, latent_dim=LATENT_DIM, base_channels=unet_channels).to(device)
    else:
...
def run_inference(data, model_path="best_model.pth", output_zarr_path=None, batch_size=DEFAULT_BATCH_SIZE, 
    ...
    if model_type == 'mlp':
        ...
        input_channels = TIME_STEPS * data.shape[-1]  # Channels = time_steps * variables
        model = WeatherUNet(input_channels, latent_dim=LATENT_DIM)
    else:
        raise ValueError(f"Unknown model type: {model_type}")
...
def main():
    ...
    parser.add_argument('--model_type', type=str, default='mlp', choices=['mlp', 'unet'],
                       help='Type of model to use (mlp or unet)')
